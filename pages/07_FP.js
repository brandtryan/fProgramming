...
cl();
cl(white, `An expression is a chunk of code that evaluates to a value.`);
cl();
...
cl(
  white,
  `Shortcut for assigning existing variables to object property keys of the same name:`
);
const a = "a";
const b = "b";
const oldA = { a: a };
...
cl(
  white,
  `Objects can be composed together into NEW objects using SPREAD operator:`
);
const c = { ...oA, ...oB };
...
cl(
  white,
  `Destructuring Object: extract values and assign to named variables:`
);
...
cl(
  white,
  `You can compare values with the strict equality operator "triple equals":`
);
...
cl(
  white,
  `Ternary expressions let you ask a question using a comparator and evaluates to different answer based on its truthyness:`
);
...
cl(
  white,
  `The math function "f(x) = 2x", used in other equations is written as f(2), where we've substituted "2" for "x". The result is "4".`
);
...
cl(
  white,
  `If you want to apply a function to some arguments, you must invoke it with a function call, which APPLIES a function to its ARGUMENTS and EVALUATES a RETURN VALUE.`
);
cl(
  yellow,
  `Invoke a function using <functionName>(argument1, argument2, etc.)`
);
cl(cyan, `double(2);`);
cl(green, double(2));
cl();
...
cl(
  white,
  `Default Parameter Values - Identity function example (function which returns the same value you pass in)`
);
cl(cyan, `const orZero = (n = 0) => n;`);
cl(
  white,
  `When you assign defaults, type inference tools (tern.js, Flow, TypeScript) can infer the type signature of your function`
);
cl(cyan, "orZero(5)");
cl(green, orZero(5));
cl();
...
cl(
  white,
  `javaScript functions can take object LITERALS as arguments and use destructuring assignment in the parameter signature to create NAMED ARGUMENTS`
);
cl(
  cyan,
  `const createUser = ({
...
cl(
  white,
  `A curried function takes multiple parameters ONE AT A TIME, returns a FUNCTION that takes next PARAMETER, and so on, until the final value is RETURNED.`
);
